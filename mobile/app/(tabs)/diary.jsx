...
export default function DiaryScreen() {
  ...
  const deleteMeal = async (mealType) => {
    try {
      ...
      const response = await fetch(
        `${API_URI}/diary/${formattedDate}/${mealType}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
    ...
    PanResponder.create({
      ...
      onPanResponderMove: (_, gesture) => {
        if (gesture.dy > 0 && !expandedCalendar) {
          ...
          calendarHeight.setValue(
            Math.min(calendarMinHeight + gesture.dy, calendarMaxHeight)
          );
        } else if (gesture.dy < 0 && expandedCalendar) {
          // Dragging up and calendar is expanded
          calendarHeight.setValue(
            Math.max(calendarMaxHeight + gesture.dy, calendarMinHeight)
          );
        }
      },
  ...
  const renderMonthView = () => {
    ...
    const scrollToCurrentWeek = () => {
      ...
      const mondayIndex = monthDates.findIndex(
        (date) =>
          date &&
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  ...
                  isSelected ? styles.selectedDate : null,
                  isToday && !isSelected ? styles.todayDate : null,
                  isCurrentWeek && !isSelected && !isToday
      ...
      <View style={styles.waterContainer}>
        <View style={styles.sectionHeaderRow}>
          ...
          <Text style={styles.waterTotal}>
            {filledCount * glassAmount} / 2000 ml
          </Text>
        </View>
        ...
        <View style={styles.waterProgressContainer}>
          ...
          <Text style={styles.waterSummary}>{filledCount} of 8 glasses</Text>
        </View>
